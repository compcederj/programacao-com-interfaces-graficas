    ...
    while not done:
      ...
      try:
         if (sys.hexversion > 0x03000000):
             ...
             word = raw_input("Player %d enter your guess: " % whoseTurn).decode("utf-8")
...
def main(argv=None):
    ...
    if len(argv) > 1:  # a file name for getting words from was given
       ...
       if len(argv) > 2:  # a secret word was also given
           ...
           game = main2(TextUI.DICTIONARY_FILENAME, secretWord) # a given secret word 
       else:
...
def main2(fwords, word):
    ...
    try:
       ...
       print (detail)
       exit('Searching word "%s" in file "%s"' % (word,TextUI.DICTIONARY_FILENAME))
    ...
    print ("A word of size %d: %s" % (TextUI.WORD_SIZE,wl.generate(TextUI.WORD_SIZE)))
